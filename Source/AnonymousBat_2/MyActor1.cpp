	...
	if (NumCubes > 0 && pCubeMeshes.Num() > 0)
	{
		...
		for (int32 j = 0; j < NumCubes; j++)
		{
			for (int32 i = 0; i < NumCubes; i++)
			{
				...
				pCubeComponent->SetStaticMesh(pCubeMeshes[1]);
...
void AMyActor1::SoundCubeVisualizer_MouseButtonDown_Implementation(UPrimitiveComponent* _ClosestHit)
{
...
void AMyActor1::SoundCubeVisualizer_MouseButtonUp_Implementation(UPrimitiveComponent* _ClosestHit)
{
